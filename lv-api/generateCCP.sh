...
sleep 5
  ...
  NO_OF_ORDERERS=1
  echo ${NO_OF_ORDERERS}
  CPWD="/home/paperspace/HLF_Group4"
  SourcePATH="/home/paperspace/HLF_Group4/lv-api"
  # CPWD="/usr/CLIF"
  ...
  T_ORGS=3
  orgDetails[0,0]="lv"
  orgDetails[1,0]="distributor"
  orgDetails[2,0]="retailer"
  orgDetails[0,2]="LV"
  orgDetails[1,2]="Distributor"
  orgDetails[2,2]="Retailer"
  orgDetails[0,1]=1
  orgDetails[1,1]=1
  orgDetails[2,1]=1
  CCC_NAME="vuittonccc"
  CHANNELS[0,0]="vuittonchannel"
IP=$(hostname -I|awk {'print $1'})
 ...
 MOPath="${CPWD}/"
  cd $MOPath
  ordererpath="${MOPath}crypto-config/ordererOrganizations/vuitton.com/orderers"



  for count in `seq 0 $Mxca`  
  do 
  Orderer_Ca[${count}]="$(awk '{printf "%s\\n", $0}' ${ordererpath}/orderer.vuitton.com/tls/ca.crt)"
...
do 
 MOPATH="${CPWD}/"
 Org_Connection[${orgDetails[${countcon},0]}cacert]="$(awk '{printf "%s\\n", $0}' ${MOPATH}crypto-config/peerOrganizations/${orgDetails[${countcon},0]}.vuitton.com/ca/ca.${orgDetails[${countcon},0]}.vuitton.com-cert.pem)"
 Org_Connection[${orgDetails[${countcon},0]}peercert]="$(awk '{printf "%s\\n", $0}' ${MOPATH}crypto-config/peerOrganizations/${orgDetails[${countcon},0]}.vuitton.com/peers/peer0.${orgDetails[${countcon},0]}.vuitton.com/tls/ca.crt)"
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json
            "peer${peercount}.${orgDetails[${peer},0]}.vuitton.com": {
            "endorsingPeer": true,
            "chaincodeQuery": true,
            "eventSource": true
                  }
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json
            "peer${peercount1}.${orgDetails[${peer},0]}.vuitton.com": {
            "endorsingPeer": true,
            "chaincodeQuery": true,
            "eventSource": true
          },
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json          
            "peer${peercountorder}.${orgDetails[${countorder},0]}.vuitton.com"
        ],
EOF
          else
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json          
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json 
        "certificateAuthorities": [
          "ca.${orgDetails[${countorder},0]}.vuitton.com"
        ]
EOF
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json 
    "orderer.vuitton.com": {
      ...
      "grpcOptions": {
        "ssl-target-name-override": "orderer.vuitton.com",
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json
      ...
      "grpcOptions": {
        "ssl-target-name-override": "peer${peercount}.${orgDetails[${peer},0]}.vuitton.com",
        "hostnameOverride": "peer${peercount}.${orgDetails[${peer},0]}.vuitton.com"
      },
      "tlsCACerts": {
...
cat <<EOF >> ${SourcePATH}/connection-${orgDetails[${countcon},2]}.json
    "ca.${orgDetails[${countco},0]}.vuitton.com": {
      "url": "http://${IP}:${ca_port}",
      "caName": "ca.${orgDetails[${countco},0]}.vuitton.com",
      "httpOptions": {
        "verify": false
      },
      "tlsCACerts": {
