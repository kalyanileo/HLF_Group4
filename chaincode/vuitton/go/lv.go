...
type Bag struct {
		...
		LesseId					string 			`json:"lesseId"`
		RentDuration 			string			`json:"rentDuration"`
...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	}else if function == "getBag" {
		return s.getBag(APIstub, args)
        }else if function == "getBagHistory" {
                return s.getBagHistory(APIstub, args)
        }else if function == "resellBag" {
                return s.resellBag(APIstub, args)
        }else if function == "rentBag" {
                return s.rentBag(APIstub, args)
        }		
...
func (s *SmartContract) assignOwner(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
		...
		bg.Price = bagID.Price
        bg.Owner = bagID.Owner
		bg.InsuranceProviderId = bagID.InsuranceProviderId
...
func (s *SmartContract) resellBag(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
        ...
        bg.Owner ="" 
	bg.Price = bagID.Price
	bg.ResellerId = bagID.ResellerId
	bg.Status = "ReSale"
        BGBytes, err := json.Marshal(bg)
        ...
        APIstub.PutState(bg.TagId,BGBytes)
        fmt.Println("Reseller Assigned -> ", bg)

        return shim.Success(nil)
}

func (s *SmartContract) rentBag(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
        ...
        //LC := LetterOfCredit{LCId: lc.LCId, ExpiryDate: lc.ExpiryDate, Buyer: lc.Buyer, Bank: lc.Bank, Seller: lc.Seller, Amount: lc.Amount, Status: "Accepted"}        
		bg.LesseId = bagID.LesseId
		bg.RentDuration = bagID.RentDuration
		bg.RentAmount = bagID.RentAmount
		bg.Status = "Rent"
        BGBytes, err := json.Marshal(bg)
        ...
        APIstub.PutState(bg.TagId,BGBytes)
        fmt.Println("Rented to Lesse -> ", bg)

        return shim.Success(nil)
}

func (s *SmartContract) getBag(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {

        bgId := args[0];



        bagLatest, _ := APIstub.GetState(bgId)


        return shim.Success(bagLatest)
